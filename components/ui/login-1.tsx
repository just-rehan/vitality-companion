  ...
  return (
    <div className="w-full min-w-[200px] relative">
      ...
      <div className="relative w-full">
        <input
          type="text"
          className="peer relative z-10 border-2 border-[var(--color-border)] h-[52px] w-full rounded-md bg-[var(--color-surface)] px-4 font-thin outline-none drop-shadow-sm transition-all duration-200 ease-in-out focus:bg-[var(--color-bg)] placeholder:font-medium text-[var(--color-text-primary)]"
...
const Page = ({ onLogin, email, setEmail, password, setPassword, isLoading }: PageProps) => {
  ...
  return (
    ...
    <div className='card w-[80%] lg:w-[70%] md:w-[55%] flex justify-between h-[600px] bg-[var(--color-surface)] rounded-3xl overflow-hidden shadow-2xl border border-[var(--color-border)]'>
      <div
        ...
        onMouseLeave={handleMouseLeave}>
          ...
          />
          <div className="form-container sign-in-container h-full z-10 relative flex items-center">
            <form className='text-center py-10 md:py-20 grid gap-2 w-full' onSubmit={(e) => { e.preventDefault(); onLogin?.(e); }}>
                      ...
                      {socialIcons.map((social, index) => {
                        return (
                          <li key={index} className="list-none">
                            <a
                              href={social.href}
                              className={`w-[2.5rem] md:w-[3rem] h-[2.5rem] md:h-[3rem] bg-[var(--color-bg-2)] rounded-full flex justify-center items-center relative z-[1] border-[3px] border-[var(--color-text-primary)] overflow-hidden group`}
                            >
                              <div
                              ...
                              />
                              <span className="text-[1.5rem] text-[hsl(203,92%,8%)] transition-all duration-500 ease-in-out z-[2] group-hover:text-[var(--color-text-primary)] group-hover:rotate-[360deg]">
                                {social.icon}
                              </span>
                            </a>
                          </li>
